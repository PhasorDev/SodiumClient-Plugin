...
public class EntityDamageEventHandler implements Listener {
    ...
    public void onEntityDamaged(EntityDamageEvent event) {
        ...
        if (entity instanceof Damageable) {
            ...
            String damage = event.getDamage() >= 1000.0 ? NumberUtils.formatCompact((long)event.getDamage()) : DECIMAL_FORMAT.format(event.getDamage());
            double health = MathHelper.clamp(Double.parseDouble(DECIMAL_FORMAT.format(damageable.getHealth() - event.getDamage())), 0.0, damageable.getMaxHealth());
            ...
            armorStand.setCustomName(ChatColor.RED + "-" + damage + "❤" + ChatColor.AQUA + "[" + health + "❤]");
